...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['tambah_peminjaman'])) {
    ...
    $sql = "INSERT INTO peminjaman (nama_peminjam, id_buku, jumlah_pinjam, tanggal_pinjam) 
            VALUES ('$nama_peminjam', $id_buku, $jumlah_pinjam, '$tanggal_pinjam')";
    
    if ($conn->query($sql)) {
        $message = "Peminjaman berhasil ditambahkan!";
    } else {
        $error = "Gagal menambahkan peminjaman: " . $conn->error;
    }
}
...
if (isset($_GET['kembalikan'])) {
    ...
    $sql = "UPDATE peminjaman SET tanggal_kembali = CURRENT_DATE() WHERE id_peminjaman = $id";
...
// Hapus Peminjaman Langsung di Sini
if (isset($_GET['hapus'])) {
    ...
    if ($conn->query($sql)) {
        ...
        $error = "Gagal menghapus data peminjaman: " . $conn->error;
    }
}
...
// Ambil Daftar Buku
$books = $conn->query("SELECT id_buku, judul FROM buku")->fetch_all(MYSQLI_ASSOC);
?>
    ...
    <div class="container">
        <div class="loan-management">
            <div class="loan-header">
                <h1 class="loan-title"><i class="fas fa-hand-holding-heart"></i> Peminjaman Buku</h1>
            </div>
            ...
            <!-- Form Peminjaman -->
            <div class="loan-form">
                <h2><i class="fas fa-plus-circle"></i> Tambah Peminjaman Baru</h2>
                <form method="POST" action="">
                    <div class="form-group">
                        <label for="nama_peminjam"><i class="fas fa-user"></i> Nama Peminjam</label>
                        <input type="text" id="nama_peminjam" name="nama_peminjam" class="form-control" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="id_buku"><i class="fas fa-book"></i> Buku</label>
                        <select id="id_buku" name="id_buku" class="form-control" required>
                            <option value="">Pilih Buku</option>
                            <?php foreach ($books as $book): ?>
                                <option value="<?= $book['id_buku'] ?>"><?= htmlspecialchars($book['judul']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    
                    <div class="form-group">
                        <label for="jumlah_pinjam"><i class="fas fa-calculator"></i> Jumlah Pinjam</label>
                        <input type="number" id="jumlah_pinjam" name="jumlah_pinjam" class="form-control" min="1" value="1" required>
                    </div>
                    
                    <div class="form-group">
                        <label for="tanggal_pinjam"><i class="fas fa-calendar-alt"></i> Tanggal Pinjam</label>
                        <input type="date" id="tanggal_pinjam" name="tanggal_pinjam" class="form-control" required value="<?= date('Y-m-d') ?>">
                    </div>
                    ...
                    <?php if (!empty($loans)): ?>
                        <?php foreach ($loans as $loan): ?>
                            <tr>
                                ...
                                <td><?= $loan['tanggal_kembali'] && $loan['tanggal_kembali'] != '0000-00-00' ? date('d/m/Y', strtotime($loan['tanggal_kembali'])) : '-' ?></td>
                                <td>
                                    <span class="status-badge <?= ($loan['tanggal_kembali'] && $loan['tanggal_kembali'] != '0000-00-00') ? 'status-dikembalikan' : 'status-dipinjam' ?>">
                                    <?= ($loan['tanggal_kembali'] && $loan['tanggal_kembali'] != '0000-00-00') ? 'Dikembalikan' : 'Dipinjam' ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="loan-actions">
                                        <?php if (!$loan['tanggal_kembali']): ?>
                                            <a href="peminjaman.php?kembalikan=<?= $loan['id_peminjaman'] ?>" class="btn-success">
                                                <i class="fas fa-undo"></i> Kembalikan
                                            </a>
                                        <?php endif; ?>
                                        <a href="peminjaman.php?hapus=<?= $loan['id_peminjaman'] ?>" class="btn-danger" onclick="return confirm('Hapus data peminjaman ini?')">
